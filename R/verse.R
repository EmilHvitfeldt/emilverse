core <- c("emilfun", "emiladdins", "emiltemplates")
...
tidyweb_attach <- function() {
  ...
  msg(
    cli::rule(
      ...
      right = paste0("tidyweb ", package_version("tidyweb"))
    ),
    startup = TRUE
  )
...
#' Conflicts between the tidyweb and other packages
#'
#' This function lists all the conflicts between packages in the tidyweb
...
#' @examples
#' tidyweb_conflicts()
tidyweb_conflicts <- function() {
  envs <- purrr::set_names(search())
  objs <- invert(lapply(envs, ls_env))

  conflicts <- purrr::keep(objs, ~ length(.x) > 1)

  pkg_names <- paste0("package:", tidyweb_packages())
  conflicts <- purrr::keep(conflicts, ~ any(.x %in% pkg_names))

  conflict_funs <- purrr::imap(conflicts, confirm_conflict)
  conflict_funs <- purrr::compact(conflict_funs)

  structure(conflict_funs, class = "tidyweb_conflicts")
}

tidyweb_conflict_message <- function(x) {
  if (length(x) == 0) return("")

  header <- cli::rule(
    left = crayon::bold("Conflicts"),
    right = "tidyweb_conflicts()"
  )
...
#' @export
print.tidyweb_conflicts <- function(x, ..., startup = FALSE) {
  cli::cat_line(tidyweb_conflict_message(x))
}
...
#' Update tidyweb packages
#'
#' This will check to see if all tidyweb packages (and optionally, their
...
#' tidyweb_update()
#' }
tidyweb_update <- function(recursive = FALSE) {

  deps <- tidyweb_deps(recursive)
  behind <- dplyr::filter(deps, behind)

  if (nrow(behind) == 0) {
    cli::cat_line("All tidyweb packages up-to-date")
    return(invisible())
  }
...
#' List all tidyweb dependencies
#'
#' @param recursive If \code{TRUE}, will also list all dependencies of
#'   tidyweb packages.
#' @export
tidyweb_deps <- function(recursive = FALSE) {
  pkgs <- utils::available.packages()
  deps <- tools::package_dependencies("tidyweb", pkgs, recursive = recursive)
...
#' List all packages in the tidyweb
#'
#' @param include_self Include tidyweb in the list?
#' @export
#' @examples
#' tidyweb_packages()
tidyweb_packages <- function(include_self = TRUE) {
  raw <- utils::packageDescription("tidyweb")$Imports
  imports <- strsplit(raw, ",")[[1]]
  parsed <- gsub("^\\s+|\\s+$", "", imports)
  names <- vapply(strsplit(parsed, "\\s+"), "[[", 1, FUN.VALUE = character(1))

  if (include_self) {
    names <- c(names, "tidyweb")
  }
...
.onAttach <- function(...) {
  ...
  if (!"package:conflicted" %in% search()) {
    x <- tidyweb_conflicts()
    msg(tidyweb_conflict_message(x), startup = TRUE)
  }
